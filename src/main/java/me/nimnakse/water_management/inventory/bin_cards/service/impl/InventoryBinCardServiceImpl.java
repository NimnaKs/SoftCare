        ...
        public PageResponse<BinCardEntryRes> getBinCard(Long inventoryTemplateId, int page, int size) {
                if (inventoryTemplateId == null) {
                        throw new BadRequestException("Inventory template id is required",
                                        "ඉන්වෙන්ටරි සැකිලි හැඳුනුම්පත අවශ්‍යයි");
                }
                if (!templateRepository.existsById(inventoryTemplateId)) {
                        throw new NotFoundException("Inventory template not found", "ඉන්වෙන්ටරි සැකිල්ල සොයාගත නොහැක",
                                        ErrorCode.NOT_FOUND);
                }
                if (size <= 0) {
                ...
                grnItems.stream()
                                ...
                                .filter(Objects::nonNull)
                                .forEach(scopedBatchNos::add);
                List<InventoryConsumption> consumptions = consumptionRepository
                                .findByInventoryTemplateId(inventoryTemplateId)
                                .stream()
